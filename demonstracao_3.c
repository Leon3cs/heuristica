...
void InserirItem(TipoItem item, TipoLista* Lista)
{
    Lista->ultimo->prox = (TipoNo*) malloc(sizeof(TipoNo));
    Lista->ultimo = Lista->ultimo->prox;
    Lista->ultimo->item = item;
    Lista->ultimo->prox = NULL;
}

void RemoverItem(TipoNo* item, TipoLista *Lista)
{ /*  ---   Obs.: o item a ser retirado e  o seguinte ao apontado por  p --- */
  TipoNo *aux;
  
  if (Vazia(*Lista) || item == NULL || item->prox == NULL) 
  { 
    printf(" Erro: lista vazia ou indice inexistente\n");
    return;
  }
